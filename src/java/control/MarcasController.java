...
public class MarcasController implements Serializable {
    ...
    @EJB
    private control.MarcasFacade ejbFacade;
    private List<Marcas> items = null;

    public List<Marcas> getItemseliminados() {
         if (items == null) {
            items = ejbFacade.Consulta_inactivos();
        }
        return items;
    }

    public void setItemseliminados(List<Marcas> itemseliminados) {
        this.itemseliminados = itemseliminados;
    }
    private List<Marcas> itemseliminados = null;
    ...
    public void destroy() {
        ...
        persist(PersistAction.UPDATE, ResourceBundle.getBundle("/Bundle").getString("MarcasDeleted"));
        if (!JsfUtil.isValidationFailed()) {
